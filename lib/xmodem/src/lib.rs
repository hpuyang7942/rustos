...
impl<T: io::Read + io::Write> Xmodem<T> {
    ...
    fn expect_byte_or_cancel(&mut self, byte: u8, expected: &'static str) -> io::Result<u8> {
        let byte_read: u8 = self.read_byte(false)?;
        if byte == byte_read {
            Ok(byte)
        }
    ...
    pub fn read_packet(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        if buf.len() < 128 {
            ...
            Err(io::Error::new(
                ...
                "buf should not be shorter than 128"
            ))
        }
        if !self.started {
            self.write_byte(NAK)?;
        ...
        match self.read_byte(true)? {
            EOT => {
                self.write_byte(NAK)?;
                self.expect_byte_or_cancel(EOT, "Expect EOT to perform end of transimssion")?;
                self.write_byte(ACK)?;
                self.started = false;
                Ok(0)
            },
            SOH => {
                ...
                self.expect_byte_or_cancel(packet, "wrong packet number")?;
                self.expect_byte_or_cancel(!packet, "wrong 1s complement of packet number")?;
    ...
    pub fn write_packet(&mut self, buf: &[u8]) -> io::Result<usize> {
        ...
        if !self.started {
            ...
            self.expect_byte(NAK, "Expect NAK for transimssion to begin")?;
            self.started = true;
            (self.progress)(Progress::Started);
        }
        ...
        if buf.is_empty() {
            self.write_byte(EOT)?;
            self.expect_byte(NAK, "Expect NAK to perform end of transmission")?;
            self.write_byte(EOT)?;
            self.expect_byte(ACK, "Expect ACK to perform end of transmission")?;
            self.started = false;
            return Ok(0)
        }
