...
impl MiniUart {
    ...
    pub fn wait_for_byte(&self) -> Result<(), ()> {
        let start = timer::Timer::new().read();
        match self.timeout {
            None => {
                loop {
                    if self.has_byte() {
            ...
            Some(time) => {
                while start + time >= timer::current_time() {
                    if self.has_byte() {
                        return Ok(());
                    }
    ...
    impl io::Read for MiniUart {
        fn read(&mut self, buf: &mut [u8]) -> Result<usize, io::Error> {
            match self.wait_for_byte() {
                Ok(()) => {
                    let mut len: usize = 0;
                    while self.has_byte() && len < buf.len() {
                        buf[len] = self.read_byte();
                        len = len + 1;
                    }
                    return Ok(len);
                },
                Err(()) => {
                    Err(io::Error::new(io::ErrorKind::TimedOut, "reading time out!"))
                }
            }
        }
    }
